...
declare v_status text; v_tx uuid;
begin
  if p_ref_table = 'payments' then
    -- Gộp milestone + upwork (cùng vòng đời draft→billed→received). Lùi 1 bước.
    select status, income_tx_id into v_status, v_tx from payments where id = p_ref_id for update;
    if not found then raise exception 'rollback_status: đợt chi trả không tồn tại'; end if;
